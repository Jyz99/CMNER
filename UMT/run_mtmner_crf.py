...
class MNERProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        data, wids, auxlabels = self._read_mmtsv(os.path.join(data_dir, "weibo-zh-train.txt"))
        return self._create_examples(data, wids, auxlabels, "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        data, wids, auxlabels = self._read_mmtsv(os.path.join(data_dir, "weibo-zh-valid.txt"))
        return self._create_examples(data, wids, auxlabels, "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        data, wids, auxlabels = self._read_mmtsv(os.path.join(data_dir, "weibo-zh-test.txt"))
    ...
    def _create_examples(self, lines, wids, auxlabels, set_type):
        ...
        for i, (sentence, label) in enumerate(lines):
            ...
            examples.append(MMInputExample(guid=guid, text_a=text_a, text_b=text_b, wid=wid, label=label, auxlabel=auxlabel))
        return examples
...
def convert_mm_examples_to_features(examples, label_list, auxlabel_list, max_seq_length, tokenizer, data_dir, img_feature):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    #         img_ids.append(files[i])
    # print('----------------------')
    # print(img_ids)
    # print(img_ids[0],type(img_ids[0]))
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # no pic
        image = img_features['0']
        # image = torch.zeros((2048, 7, 7))

        # for i in range(len(img_ids)):
        #     if img_ids[i].startswith(wid):
        #         image_path = os.path.join(path_img, img_ids[i])
        #         if not os.path.exists(image_path):
        #             print(image_path)
        #         else:
        #             try:
        #                 temp = image_process(image_path, transform)
        #                 cnt += 1
        #                 image += temp
...
def main():
    ...
    parser.add_argument("--img_feature_file", default="resnet_img_features_motley.pth", type=str,help="Filename for preprocessed image features")
    ...
    if args.do_train:
        train_features = convert_mm_examples_to_features(
            train_examples, label_list, auxlabel_list, args.max_seq_length, tokenizer, args.data_dir, args.img_feature_file)
        ...
        eval_features = convert_mm_examples_to_features(
            eval_examples, label_list, auxlabel_list, args.max_seq_length, tokenizer, args.data_dir, args.img_feature_file)
        ...
        test_eval_features = convert_mm_examples_to_features(
            test_eval_examples, label_list, auxlabel_list, args.max_seq_length, tokenizer, args.data_dir, args.img_feature_file)
        ...
        for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                # print('HELLO', img_feats.shape)  # torch.Size([32, 3, 224, 224])
                # with torch.no_grad():
                #     imgs_f, img_mean, img_att = encoder(img_feats)
                # print("img_att!!!")
                # print(type(img_att),img_att.size())
                trans_matrix = torch.tensor(trans_matrix).to(device)
                # print('HELLO', trans_matrix.float().shape)
            ...
            for input_ids, input_mask, added_input_mask, segment_ids, img_feats, label_ids, auxlabel_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                with torch.no_grad():
                    # imgs_f, img_mean, img_att = encoder(img_feats)
                    predicted_label_seq_ids = model(input_ids, segment_ids, input_mask, added_input_mask, img_feats, trans_matrix)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        sentence_list = []
        test_data, imgs, _ = processor._read_mmtsv(os.path.join(args.data_dir, "weibo-zh-test.txt"))
