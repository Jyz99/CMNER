...
class MTCCMBertForMMTokenClassificationCRF(BertPreTrainedModel):
    ...
    def forward(self, input_ids, segment_ids, input_mask, added_attention_mask, visual_embeds_att, trans_matrix,
        ...
        sequence_output, _ = self.bert(input_ids, token_type_ids=segment_ids, attention_mask=input_mask,
                                       output_all_encoded_layers=False)    #(32,250,768)
        sequence_output = self.dropout(self.map(sequence_output))   #r

        extended_txt_mask = input_mask.unsqueeze(1).unsqueeze(2)   #(32,1,1,250)
        ...
        trans_bert_feats = torch.matmul(aux_bert_feats, trans_matrix.float())  #（32,250,13） 2048*7，7*13

        main_addon_sequence_encoder = self.self_attention_v2(sequence_output, extended_txt_mask)
        main_addon_sequence_output = main_addon_sequence_encoder[-1]   #(32,250,768)

        vis_embed_map = visual_embeds_att.view(-1, 49, 2048)   #v（32,49,2048）
        # vis_embed_map = visual_embeds_att.view(-1, 2048, 49).permute(0, 2, 1)  # self.batch_size, 49, 2048
        converted_vis_embed_map = self.vismap2text(vis_embed_map)  #(32,49,768) self.batch_size, 49, hidden_dim

        # '''
        # apply txt2img attention mechanism to obtain image-based text representations
        # img_mask = added_attention_mask[:, :49]
        img_mask = added_attention_mask[:, :49]   #(32,49)
        extended_img_mask = img_mask.unsqueeze(1).unsqueeze(2)   #(32,1,1,49)
        extended_img_mask = extended_img_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_img_mask = (1.0 - extended_img_mask) * -10000.0

        cross_encoder = self.txt2img_attention(main_addon_sequence_output, converted_vis_embed_map, extended_img_mask)   #(32,250,768)
        cross_output_layer = cross_encoder[-1]  #(32,250,768) self.batch_size * text_len * hidden_dim

        # apply img2txt attention mechanism to obtain multimodal-based text representations
        converted_vis_embed_map_v2 = self.vismap2text_v2(vis_embed_map)  #(32,49,768) self.batch_size, 49, hidden_dim

        cross_txt_encoder = self.img2txt_attention(converted_vis_embed_map_v2, main_addon_sequence_output, extended_txt_mask)
        cross_txt_output_layer = cross_txt_encoder[-1]  #(32,49,768) self.batch_size * 49 * hidden_dim
        cross_final_txt_encoder = self.txt2txt_attention(main_addon_sequence_output, cross_txt_output_layer, extended_img_mask)   #(32,250,768)
        ##cross_final_txt_encoder = self.txt2txt_attention(aux_addon_sequence_output, cross_txt_output_layer, extended_img_mask)
        cross_final_txt_layer = cross_final_txt_encoder[-1]  #(32,250,768) self.batch_size * text_len * hidden_dim  a
        #cross_final_txt_layer = torch.add(cross_final_txt_layer, sequence_output)

        # visual gate
        merge_representation = torch.cat((cross_final_txt_layer, cross_output_layer), dim=-1)   #(32,250,768*2)
        gate_value = torch.sigmoid(self.gate(merge_representation))  ##(32,250,768) batch_size, text_len, hidden_dim
        gated_converted_att_vis_embed = torch.mul(gate_value, cross_output_layer)   ##(32,250,768) b
        ...
        final_output = torch.cat((cross_final_txt_layer, gated_converted_att_vis_embed), dim=-1)   #(32,250,768*2)
        ...
        bert_feats = self.classifier(final_output)   #(32,250,13)

        alpha = 0.5
        final_bert_feats = torch.add(torch.mul(bert_feats, alpha),torch.mul(trans_bert_feats, 1-alpha))   #(32,250,13)
        ...
        if labels is not None:
            ...
            ##beta = 0.6
            aux_loss = - self.aux_crf(aux_bert_feats, auxlabels, mask=input_mask.byte())
            main_loss = - self.crf(final_bert_feats, labels, mask=input_mask.byte())
