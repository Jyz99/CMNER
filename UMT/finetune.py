...
class MMInputExample(object):
    ...
    def __init__(self, guid, text_a, text_b, wid, label=None, auxlabel=None):
        """Constructs a InputExample.
...
def mmreadfile(filename):
    ...
    print("The number of samples: " + str(len(data)))
    print("The number of wids: " + str(len(wids)))
    return data, wids, auxlabels
...
class MNERProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        data, wids, auxlabels = self._read_mmtsv(os.path.join(data_dir, "twitter-en-train.txt"))
        return self._create_examples(data, wids, auxlabels, "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        data, wids, auxlabels = self._read_mmtsv(os.path.join(data_dir, "twitter-en-valid.txt"))
        return self._create_examples(data, wids, auxlabels, "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        data, wids, auxlabels = self._read_mmtsv(os.path.join(data_dir, "twitter-en-test.txt"))
        return self._create_examples(data, wids, auxlabels, "test")
    ...
    def _create_examples(self, lines, wids, auxlabels, set_type):
        examples = []
        for i, (sentence, label) in enumerate(lines):
            ...
            examples.append(
                MMInputExample(guid=guid, text_a=text_a, text_b=text_b, wid=wid, label=label, auxlabel=auxlabel))
        return examples
...
def convert_mm_examples_to_features(examples, label_list, auxlabel_list, max_seq_length, tokenizer, data_dir,
                                    img_feature):
    """Loads a data file into a list of `InputBatch`s."""

    label_map = {label: i for i, label in enumerate(label_list, 1)}
    auxlabel_map = {label: i for i, label in enumerate(auxlabel_list, 1)}
    img_features = torch.load(os.path.join(data_dir, img_feature))
...
def main():
    ...
    parser.add_argument("--img_feature_file", default="resnet_img_features_motley.pth", type=str,
    ...
    elif args.task_name == "weibo":
        args.path_image = "./data/weibo_images/"
    ...
    if args.task_name == "weibo":
        args.num_train_epochs = 35.0
    # '''
    ...
    if os.path.exists(args.finetune_dir) and os.listdir(args.finetune_dir) and args.do_finetune:
        raise ValueError("Finetune directory ({}) already exists and is not empty.".format(args.finetune_dir))
    if not os.path.exists(args.finetune_dir):
    ...
    # ''' initialization of our conversion matrix, in our implementation, it is a 7*13 matrix initialized as follows:
    ...
    # 冻结 BERT 模型的 embedding 层和底下三层模型参数
    ...
    if args.do_finetune:
        ...
        train_features = convert_mm_examples_to_features(
            train_examples, label_list, auxlabel_list, args.max_seq_length, tokenizer, args.data_dir,
            args.img_feature_file)
        ...
        all_img_feats = torch.stack([f.img_feat for f in train_features])  # 3*224*224
        ...
        eval_features = convert_mm_examples_to_features(
            eval_examples, label_list, auxlabel_list, args.max_seq_length, tokenizer, args.data_dir,
            args.img_feature_file)
        ...
        test_eval_features = convert_mm_examples_to_features(
            test_eval_examples, label_list, auxlabel_list, args.max_seq_length, tokenizer, args.data_dir,
            args.img_feature_file)
        ...
        for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                neg_log_likelihood = model(input_ids, segment_ids, input_mask, added_input_mask,
                                           img_feats, trans_matrix, label_ids, auxlabel_ids).mean()
                if n_gpu > 1:
            ...
            for input_ids, input_mask, added_input_mask, segment_ids, img_feats, label_ids, auxlabel_ids in tqdm(
                ...
                with torch.no_grad():
                    predicted_label_seq_ids = model(input_ids, segment_ids, input_mask, added_input_mask, img_feats,
                                                    trans_matrix)
            ...
            if F_score_dev > max_dev_f1:
                ...
                torch.save(model_to_save.state_dict(), finetune_model_file)
                ...
                model_config = {"bert_model": args.bert_model, "do_lower": args.do_lower_case,
                                ...
                                "label_map": label_map}
                json.dump(model_config, open(os.path.join(args.finetune_dir, "model_config.json"), "w"))
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        sentence_list = []
        test_data, imgs, _ = processor._read_mmtsv(os.path.join(args.data_dir, "twitter-en-test.txt"))
        output_pred_file = os.path.join(args.finetune_dir, "mtmner_pred.txt")
        ...
        output_eval_file = os.path.join(args.finetune_dir, "eval_results.txt")
