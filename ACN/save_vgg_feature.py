...
if __name__ == "__main__":
    ...
    parser.add_argument("--img_dir", default="weibo_images", type=str, help="Path for img dir")
    parser.add_argument("--feature_file", default="img_vgg_features_motley.pt", type=str, help="Filename for preprocessed image features")
    ...
    img_id_lst = []
    for root,dirs,files in os.walk(os.path.join(args.data_dir, args.img_dir),topdown=False):
        for i in range(len(files)):
            img_id_lst.append(files[i])
    ...
    #                 img_id_lst.append(line.strip().split('IMGID:')[1])
    ...
    for idx, img_id in enumerate(img_id_lst):
        img_path = os.path.join(args.data_dir, args.img_dir, '{}'.format(img_id))
        try:
